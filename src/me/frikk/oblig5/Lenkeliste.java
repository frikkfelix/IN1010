...
class Lenkeliste<T> implements Liste<T> {
    ...
    /**
     * Definerer Iterator-objektet som skal iterere gjennom listen
     */
    class LenkelisteIterator implements Iterator<T> {
        private int index = 0;
        private Lenkeliste<T> skalItereres;

        public LenkelisteIterator(Lenkeliste<T> skalItereres) {
            this.skalItereres = skalItereres;
        }

        @Override
        public boolean hasNext() {
            return index < skalItereres.stoerrelse();
        }
    
        @Override
        public T next() {
            return skalItereres.hent(index++);
        }
